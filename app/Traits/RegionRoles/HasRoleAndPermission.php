...
trait HasRoleAndPermission
{
    ...
    {
        return $this->belongsToMany(config('model.region_roles.region_role.model.model'),'region_user_region_role')->withTimestamps();
    }
    ...
    {
        $permissionModel = app(config('model.region_roles.region_permission.model.model'));

        if (!$permissionModel instanceof Model) {
            throw new InvalidArgumentException('[model.region_roles.region_permission.model.model] must be an instance of \Illuminate\Database\Eloquent\Model');
        }

        return $permissionModel::select(['region_permissions.slug', 'region_permissions.name'])
            ->join('region_permission_region_role', 'region_permission_region_role.region_permission_id', '=', 'region_permissions.id')
            ->join('region_roles', 'region_roles.id', '=', 'region_permission_region_role.region_role_id')
            ->whereIn('region_roles.id', $this->getRoles()->pluck('id')->toArray())
            ->orWhere('region_roles.level', '<', $this->level());
            //->groupBy(['region_permissions.slug']);
    }
    ...
    public function menus($parent_id = 0)
    {
        ...
        return $permissionModel::select(['region_permissions.slug', 'region_permissions.name','region_permissions.icon'])
            ->join('region_permission_region_role', 'region_permission_region_role.region_permission_id', '=', 'region_permissions.id')
            ->join('region_roles', 'region_roles.id', '=', 'region_permission_region_role.region_role_id')
            ->where('is_menu', 1)
    ...
    {
        return null;
        return $this->belongsToMany(config('model.region_roles.region_permission.model.model'))->withTimestamps();
    }
